...
chekhov %>% 
  ...
  mutate(beta_prior = n_words-n,
         ...
         low_ci = binom.test(x = n, n = n_words)$conf.int[1],
         up_ci = binom.test(x = n, n = n_words)$conf.int[2]) %>% 
  ggplot(aes(x= trunc_titles))+
  geom_pointrange(aes(y = average, ymin = low_ci, ymax = up_ci), color = "tomato")+
  geom_pointrange(aes(y = average_post, ymin = cred_int_l, ymax = cred_int_h), color = "darkturquoise", position = "jitter")+
  coord_flip()+
  labs(title = 'Доверительные интервалы употребления "не" в рассказах А. Чехова',
       x = "", y = "",
       caption = "красные --- фриквентистский; голубой --- байесовский")
```
