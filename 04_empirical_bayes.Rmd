...
Оцените параметры бета распределения для данных при помощи эмпирической байесовской оценки. Получив параметры априорного бета распределения, проведите байесовский апдейт для каждого языка. Посчитайте модуль разницы между апостериорных долей и изначальных долей представленной в данных. В ответе приведите модуль разницы для языка `Acoli` с точностью до трех знаков после запятой.
