...
### 2.1
Вашего друга похитили а на почту отправили [датасет](https://raw.githubusercontent.com/agricolamz/2019_BayesDan_winter/master/datasets/weather.csv), в котором записаны данные о погоде из пяти городов. Ваш телефон зазвонил, и друг сказал, что не знает куда его похитили, но за окном легкий дождь (`Rain`). А на следующий день --- сильный дождь (`Thunderstorm`). Сделайте байесовский апдейт и предположите, в какой город вероятнее всего похитили друга (форма принимает латиницу).
...
Проведите байесовский апдейт наблюдений каждого языка, используя в качестве априорного распределения бета распределение с параметрами α = 9.300246, и β = 4.4545. Посчитайте модуль разницы между апостериорной долей и изначальной долей представленной в данных. В ответе укажите язык с наибольшей разницей.
...
results %>% 
  ...
  geom_point()+
  labs(title = "изменеие вероятностей для каждой из моделей",
       x = "μ")
```
...
В [датасет](https://raw.githubusercontent.com/agricolamz/2019_BayesDan_winter/master/datasets/phoible_n_consonants.csv) записаны данные по исследованию мочи [Andrews, Herzberg 1985]. В переменной `pH` записано значение уровня кислотности. Сделайте байесовский апдейт наблюдаемых данных, используя априорное распределение $\mathcal{N}(\mu = 9, \sigma^2 = 2)$.
